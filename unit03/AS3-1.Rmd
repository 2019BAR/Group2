...
<br>
<span style="color:blue;">Month/Day/Year Hour:Minute</span>
```{r}
head(mvt$Date)
mvt$Date= as.POSIXct(mvt$Date,format="%m/%d/%y %H:%M")#轉換成格式POSIXct
```
<br>
<span style="color:blue;">Day/Month/Year Hour:Minute</span>
```{r}
format(mvt$Date,"%d/%m/%y %H:%M")%>%head()#format排列head顯示
```
<br>
<span style="color:blue;">Hour:Minute Month/Day/Year</span>
```{r}
format(mvt$Date,"%H:%M %m/%d/%y")%>%head()#同上
```
<br><span style="color:blue;">Hour:Minute Day/Month/Year</span>
```{r fig.height=2.5, fig.width=8}
format(mvt$Date,"%H:%M %d/%m/%y")%>%head()#同上
```
...
```{r}
table(format(mvt$Date,'%m'))%>%sort
#table與format整理各月失竊車數並用sort排列找出最少失竊量月份
```

【2.4】 On which weekday did the most motor vehicle thefts occur?
```{r}
table(format(mvt$Date,'%w'))%>%sort
#同2.3
```
...
ts[mvt$Arrest]%>%format('%m')%>%table()%>%sort
#找出已逮捕之案件並整理排列
```
...
table(ts[mvt$Arrest]>as.POSIXct("2007-01-01"))
#2007年前已逮捕數量較2007年後的多
```

【3.3】 For what proportion of motor vehicle thefts in 2001 was an arrest made?
```{r}
table(mvt$Arrest,format(ts,'%Y')=="2001")%>%prop.table()%>%round(3)
#prop.table計算百分比 上方TRUE為2001年比例 FALSE為其它年比例

```

【3.4】 For what proportion of motor vehicle thefts in 2007 was an arrest made?
```{r}
table(mvt$Arrest,format(ts,'%Y')=="2007")%>%prop.table()%>%round(3)
#同3.3
```

【3.5】 For what proportion of motor vehicle thefts in 2012 was an arrest made?
```{r}
table(mvt$Arrest,format(ts,'%Y')=="2012")%>%prop.table()%>%round(3)
#同3.3
```
...
```{r}
table(mvt$LocationDescription)%>%sort%>%tail(6)
#排列後取最後幾項取得五個較大的地區失竊數量
```

【4.2】 How many observations are in Top5?
```{r}
(Top5 = (table(mvt$LocationDescription)%>%sort%>%tail%>%names)[-4])
#紀錄最高的五個地區
...
```{r}
ts[mvt$LocationDescription=="GAS STATION"]%>%format("%w")%>%table%>%sort
#挑出位在加油站失竊通報時間再以周作整理排列
```

【4.5】 On which day of the week do the fewest motor vehicle thefts in residential driveways happen?
```{r}
ts[mvt$LocationDescription=="DRIVEWAY - RESIDENTIAL"]%>%format("%w")%>%table%>%sort
#同4.4
```
