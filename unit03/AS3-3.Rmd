...
#使用table函數
table(CPS$Industry)
#可看出Educational and health services是最普遍的受訪者的行業
...
```{r}
#利用table抓取CPS中受訪者的公民身份的欄位
table(CPS$Citizenship!="Non-Citizen")%>%prop.table
#Proportion of interviewees are citizens of the United States is 0.942.
```
...
#American Indian, Black, Multiracial, White (at least 250 interviewees) 
```
...
```{r}
#can be read by using summary
summary(CPS)
#MetroAreaCode NA's:34238
#Married NA's:25338 
#Education NA's:25338
#EmploymentStatus NA's:25789
#Industry NA's:65060
```
...
```{r}
#承上表
# 3:District of Columbia, New Jersey, Rhode Island
```
...
#34.8% in the Midwest (the largest propotion of interviewees living in a non-metropolitan area)
```

__§ 2.5__ Which state has a proportion of interviewees living in a non-metropolitan area closest to 30%?
```{r}
tapply(is.na(CPS$MetroAreaCode), CPS$State, mean)
#Wisconsin is closest to 30%( 0.299)
```
...
#After ignoring all non-metropolitan states, Montana  is the largest.
```
...
```{r}
#運用merge的函式可以把metro的資料併入CPS內
CPS = merge(CPS, MetroAreaMap, by.x="MetroAreaCode", by.y="Code", all.x=TRUE)
str(CPS)
#MetroArea (the name of the variable that was added to the data frame)
```

How many interviewees have a missing value for the new metropolitan area variable?
```{r}
summary(CPS)
#There are 34238 values that have NA's values.
```
...
```{r}
#the average amount of Hispanic sorted by MetroArea
...
```{r}
#no high school diploma可從education查找
#CPS$Education == "No high school diploma"
#need to ignore missing values(use  na.rm=TRUE)
sort(tapply(CPS$Education == "No high school diploma", CPS$MetroArea, mean, na.rm=TRUE))%>%head
#Iowa City, IA(2.9%) has the smallest proportion of interviewees who have received no high school diploma.
```
...
```{r}
summary(CPS)
#New country of birth has 176 missing values.
```
...
```{r}
#在table中放入題目所需條件
#1.interviewees from the "New York-Northern New Jersey-Long Island, NY-NJ-PA"
#2.have a country of birth that is not the United States
table(CPS$MetroArea == "New York-Northern New Jersey-Long Island, NY-NJ-PA", CPS$Country != "United States")
#方法2：tapply(CPS$Country!="United States",CPS$MetroArea=="New York-Northern New Jersey-Long Island, NY-NJ-PA",mean,na.rm=TRUE)
```
...
```{r}
#Sum is used to count numbers of a particular country in a particular metropolitan area.
sort(tapply(CPS$Country == "India", CPS$MetroArea, sum, na.rm=TRUE))%>%tail
#NY-NJ-PA has 96 interviewees born in India.
```

In Brazil?
```{r}
sort(tapply(CPS$Country == "Brazil", CPS$MetroArea, sum, na.rm=TRUE))%>%tail
#Boston-Cambridge-Quincy, MA-NH has 18 interviewees born in Brazil.
```

In Somalia?
```{r}
sort(tapply(CPS$Country == "Somalia", CPS$MetroArea, sum, na.rm=TRUE))%>%tail
#Minneapolis-St Paul-Bloomington, MN-WI  has 17 interviewees born in Somalia.
```
