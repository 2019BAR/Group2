...
- - -
```{r}
CPSD=read.csv("data/CPSData.csv")
...
```{r}
table(CPSD$Industry)%>%sort#or summary(CPSD)
#Educational and health services 
```

__§ 1.3__ Which state has the fewest interviewees?
```{r}
sort(table(CPSD$State))
#New Mexico
```
...
```{r}
table(CPSD$Citizenship!="Non-Citizen")%>%prop.table
#0.94219
```
...
```{r}
table(CPSD$Age,is.na(CPSD$Married))
#Age
```
...
```{r}
table(CPSD$State,is.na(CPSD$MetroAreaCode))
#2
```
...
```{r}
CPSD=merge(CPSD,MAC,by.x="MetroAreaCode",by.y="Code",all.x=TRUE)
str(CPSD)
#MetroArea
...
```{r}
tapply(CPSD$Race=="Asian",CPSD$MetroArea,mean)%>%sort%>%tail
#4
```
...
```{r}
tapply(CPSD$Education == "No high school diploma", CPSD$MetroArea, mean,na.rm=TRUE)%>%sort%>%head
#Iowa City, IA
```
...
```{r}
CPSD=merge(CPSD,CC,by.x="CountryOfBirthCode",by.y="Code",all.x=TRUE)
str(CPSD)
#Country
```
...
```{r}
#在NY-NJ-PA中不是美國人的平均人數
tapply(CPSD$Country!="United States",CPSD$MetroArea=="New York-Northern New Jersey-Long Island, NY-NJ-PA",mean,na.rm=TRUE)
#0.3087
```
...
```{r}
tapply(CPSD$Country=="India",CPSD$MetroArea,sum,na.rm=TRUE)%>%sort%>%tail
#New York-Northern New Jersey-Long Island, NY-NJ-PA
```

In Brazil?
```{r}
tapply(CPSD$Country=="Brazil",CPSD$MetroArea,sum,na.rm=TRUE)%>%sort%>%tail
#Boston-Cambridge-Quincy, MA-NH 
```

In Somalia?
```{r}
tapply(CPSD$Country=="Somalia",CPSD$MetroArea,sum,na.rm=TRUE)%>%sort%>%tail
