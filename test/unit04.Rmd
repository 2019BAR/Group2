...
lm(formula=rated~rating,data=rate[-c(1,5,17,18),])
cor.test(rate[-c(1,5,17,18),]$rating,rate[-c(1,5,17,18),]$rated)

#從相關分析中可以得知所有組別的得給分負相關不太明顯只有-0.12 pvalue=0.59不顯著
#將資料會成圖後可以發現有三群主要群體分別為給分高得分低 給低得高 給得相當 與偏差值較大的數值
#先把1,17,18去除後取得較低的相關係數也有較顯著的結果
```
...
ggplot(rate,aes(rated,rating,label=team))+
 geom_abline()+geom_text()+geom_smooth(method="lm",se=F)+labs(x="得分",y="給分")
