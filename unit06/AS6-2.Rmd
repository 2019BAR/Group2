...
```{r}
#是指變數的值無法預先確定僅以一定的可能性(概率)取值的量。它是由於隨機而獲得的非確定值，是概率中的一個基本概念。
#測量具隨機性、偶然性的實驗。某一事件在相同的條件下可能發生也可能不發生的事件，例如某段時間回復的電話數量等。
```
...
```{r}
#都是一種模型。因為直方圖和數值標記都能將現實抽象化，使其好懂(好溝通)或好計算，而且使用者也能依自己需求調整參數來改變模型。
```
...
```{r}
#計算不同區間大小的期望值
integrate(PDF, 4.2, 4.52)$value*100-10
integrate(PDF, 4.2, 4.7)$value*100-15
integrate(PDF, 4.1, 4.77)$value*100-20
(integrate(PDF,1.8,1.96)$value+integrate(PDF, 4.1, 4.77)$value)*100-25
(integrate(PDF,1.8,2.12)$value+integrate(PDF, 4.1, 4.77)$value)*100-30
#Cst EV   earn 
#10 7.9  17.9   
#15 12.3 27.8   
#20 15.2 35.2   
...
#Cst EV   earn  期望投資報酬率
#10 7.9  17.9   1.79
#15 12.3 27.8   1.85333
#20 15.2 35.2   1.76
#25 18.1 43.1   1.724
#30 20.23 50.23 1.67
c(50.23,43.1,35.2,27.8,17.9)/c(30,25,20,15,10)
