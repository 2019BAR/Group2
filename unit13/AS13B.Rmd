...
```{r}
glm2 = glm(PoorCare ~ ., TR[,-1], family=binomial)
pred1 = predict(glm2, type = "response")
hist(pred)
abline(v=0.5, col='red')
...
```{r}
par(mfrow=c(1,2), cex=0.8)
trAUC = colAUC(pred1, y=TR$PoorCare, plotROC=T)
tsAUC = colAUC(pred2, y=TS$PoorCare, plotROC=T)
```

&emsp; 【C】 分別算出`Training`和`Testing`的`ACC`、`SENS`和`SPEC`<br>
```{r}
z1 = table(Act = TR$PoorCare, Predict = pred1 > 0.5)
z2 = table(Act = TS$PoorCare, Predict = pred2 > 0.5)
sapply(list(train = z1, test = z2), A2x2)
```
...
c(trAUC, tsAUC)
```
&emsp; 【E】 跟用兩個預測變數的模型相比，這一個模型有比較準嗎？ 有<br>
&emsp; 【F】 為什麼它比較準(或比較不準)呢？ 在樣本內的誤差通常會越低，但可能會造成過度適配<br><br>
</p class="qiz">
